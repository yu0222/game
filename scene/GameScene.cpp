...
void GameScene::PlayerUpdate() {
	...
	if (input_->PushKey(DIK_DOWN) && worldTransformPlayer_.translation_.y == 3.0f) {
...
void GameScene::BeemBorn() {
	if (BeemTimer == 0) {
		...
		for (int i = 0; i < 10; i++) {
			if (BeemFlag_[i] == 0) {
				if (input_->PushKey(DIK_SPACE) && worldTransformPlayer_.translation_.y==0) {
	...
	for (int i = 0; i < 10; i++) {
		if (EnemyFlag_[i] == 1) {
			for (int k = 0; k < 10; k++) {
				...
				float dy = abs(
				    worldTransformBeem_[k].translation_.y - worldTransformEnemy_[i].translation_.y);
				if (BeemFlag_[k] == 1) {
					if (dx < 1 && dz < 1 && dy < 1) {
						EnemyFlag_[i] = 2;
						enemyJumpSpeed_[i] = 1;
